...
import Component from '../RequestsJobsList'

describe('RequestsJobsList', () => {
  const defaultProps = {
    data: [],
  ...
  it('render snapshot', () => {
    ...
    const renderHiddenItems = wrapper
      .instance()
      .renderHiddenItems({ item: { _id: '123ewq' } })
  ...
  it('accept method', done => {
    ...
    wrapper.instance().acceptedItem = '41er23r21f1'
    ...
    const args = [
      ...
      config.SwipeListView.rightOpenValue,
    ]
    ...
    setTimeout(() => {
      ...
      expect(props.onAccept).toHaveBeenCalledWith({ _id: '41er23r21f1' })
      done()
    }, 1100)
  })

  it('set action item id', () => {
    const wrapper = shallow(<Component {...defaultProps} />)

    expect(wrapper.instance().removedItemId).toBeNull()
    expect(wrapper.instance().acceptedItem).toBeNull()
    ...
    expect(wrapper.instance().acceptedItem).toBe('41er23r21f1')
